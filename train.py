...
def get_parameters():
    ...
    parser.add_argument('--train_all_epoch', type=int, default=32)

    # DATA_DETAIL
    parser.add_argument('--img_train_path', type=str, default='../../../../data/Potsdam_Original/train/label/')
    parser.add_argument('--img_test_path', type=str, default='../../../../data/Potsdam_Original/test/label/')
...
def main(config):
    ...
    train_model.load_pkl(config.train_pkl_path, mode='trained', load=True)
    loss_fun = nn.L1Loss()
    ...
    # 2 train
    for epoch in range(train_model.train_epoch, config.train_all_epoch + 1):
...
def test(config, epoch, best_psnr, model, training=True):
    ...
    test_counter = fjn_util.Model_Statistics(config.log_path,
                                             ['Test_psnr', 'Test_ssim', 'Test_mae', 'Test_mse', 'Test_pearsonr_corr','Test_sam', 'Test_ergas'])
    ...
    if not training:
        ...
        '''
        value_list = [test_counter.average_count['Test_psnr'].avg, test_counter.average_count['Test_ssim'].avg, test_counter.average_count['Test_mse'].avg,
                      est_counter.average_count['Test_mae'].avg, test_counter.average_count['Test_pearsonr_corr'].avg,

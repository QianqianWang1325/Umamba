...
def get_parameters():
    ...
    parser.add_argument('--img_test_path', type=str, default='../../../../data/Potsdam_Original/test/ms/')
    ...
    parser.add_argument('--best_pkl_path', type=str, default='./log/pkl/best/best-0020.pkl', required=True)
...
def main(config):
    ...
    test_counter = fjn_util.Model_Statistics(config.log_path,['Test_psnr', 'Test_ssim', 'Test_mae', 'Test_mse', 'Test_pearsonr_corr','Test_sam', 'Test_ergas', 'Test_cc', 'Test_uiqi', 'Test_lpips'])
    ...
    with tqdm(total=len(test_data_loader), ascii=True) as t:
        ...
        for step, (label, inpu, img_name) in enumerate(test_data_loader):
            ...
            cv2.imwrite(config.result_path + img_name, out_bgr_hr_cpu)

            (psnr, ssim, mse, mae, pearsonr_corr, lpips) = fjn_util.cal_all_index(img_bgr, out_bgr_hr_cpu)
            (sam, cc, scc, uiqi, rmse, ergas) = iqa.cal_all_fusioneval_r(img_bgr, out_bgr_hr_cpu)
            ...
            test_counter.average_count['Test_sam'].update(sam)
            test_counter.average_count['Test_ergas'].update(ergas)
            test_counter.average_count['Test_cc'].update(cc)
            test_counter.average_count['Test_uiqi'].update(uiqi)
            test_counter.average_count['Test_lpips'].update(lpips)
    ...
    print('sam', test_counter.average_count['Test_sam'].avg)
    print('ergas', test_counter.average_count['Test_ergas'].avg)
    print('cc', test_counter.average_count['Test_cc'].avg)
    print('uiqi', test_counter.average_count['Test_uiqi'].avg)
    print('lpips', test_counter.average_count['Test_lpips'].avg)

    value_list = [test_counter.average_count['Test_psnr'].avg, test_counter.average_count['Test_ssim'].avg,
                  test_counter.average_count['Test_mse'].avg,
                  test_counter.average_count['Test_mae'].avg, test_counter.average_count['Test_pearsonr_corr'].avg,
                  test_counter.average_count['Test_sam'].avg, test_counter.average_count['Test_ergas'].avg,
                  test_counter.average_count['Test_cc'].avg,test_counter.average_count['Test_uiqi'].avg,
                  test_counter.average_count['Test_lpips'].avg]
    excel_write(begin_row=2, begin_line=12, value_list=value_list)

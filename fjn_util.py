...
def numpy_to_tensor(img):
    # 如果图像是 numpy 数组，将其转换为 torch.Tensor
...
def cal_lpips(img1, img2):
    # 将输入图像转换为 Tensor
    img1 = numpy_to_tensor(img1)
    img2 = numpy_to_tensor(img2)
...
def cal_all_index(img1, img2) -> list:
    ...
    assert img1.shape == img2.shape, 'shape doesnt match'
    assert len(img1.shape) in [2, 3], 'shape doesnt match'
    return [psnr(img1, img2), ssim(img1, img2), mse(img1, img2), mae(img1, img2), pearsonr_corr(img1, img2), cal_lpips(img1, img2)]
